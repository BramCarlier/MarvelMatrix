<script setup lang="ts">
    ...
    function isExpanded(id: string) {
        return expandedChars.value.has(id)
    }
    ...
    onUnmounted(() => {
        if (ro && timelineWrap.value) ro.unobserve(timelineWrap.value)
    })
    ...
    <div class="container">
            ...
            <div class="tItem" v-for="it in sorted" :key="it.id" :data-id="it.id">
                ...
                <div class="card">
                    ...
                    <div class="stones" v-if="it.infinityStones.length">
                        ...
                        <span v-for="st in it.infinityStones" :key="it.id+st" class="stoneDot" :title="st" :style="{
        ...
        <div v-else-if="view === 'release'" class="grid">
            <div class="card" v-for="it in sorted" :key="it.id">
                ...
                <div class="syn">{{ it.synopsis }}</div>
                <div class="characters">
                    <span v-for="c in isExpanded(it.id) ? it.characters : it.characters.slice(0,16)" :key="it.id+c" class="chip" :class="{active: state.chars.has(c)}" @click="chipToggleCharacter(c)">{{ c }}</span>
            ...
            <div v-if="showFilters" class="backdrop" @click.self="showFilters=false" role="dialog" aria-modal="true">
                <div class="panel">
                    ...
                    <div>
                        ...
                        <div class="row">
                            <button class="pill" v-for="p in allPhases" :key="p" @click="toggleSet(state.phases, p)" :style="{outline: state.phases.has(p) ? '2px solid rgba(240,19,30,.35)' : 'none'}">
                                {{ p }}
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="row" style="margin-top:8px">
                            <button class="pill" v-for="s in allStories" :key="s" @click="toggleSet(state.stories, s)" :style="{outline: state.stories.has(s) ? '2px solid rgba(43,124,255,.35)' : 'none'}">
                                {{ s }}
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="row" style="margin-top:10px">
                            <button class="pill" v-for="c in allChars.slice(0,80)" :key="c" @click="toggleSet(state.chars, c)" :style="{outline: state.chars.has(c) ? '2px solid rgba(240,19,30,.35)' : 'none'}">
                                {{ c }}
                            </button>
                        </div>
